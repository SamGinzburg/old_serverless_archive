...
function bench(cb) {
    s3.getObject({
            Bucket: 'serverless-test-' + process.env['AWS_REGION'],
            Key: 'conversation.mov'
        }, function(err, data) {
            if (err) {
                ...
                var result = spawnSync('/tmp/mbench', ['-c', '32768', '-n', '150'] ,{
                    cwd: process.cwd(),
                    env: process.env,
                    stdio: 'pipe',
                    encoding: 'utf-8'
...
exports.handler = function(event, context, callback) {
    ...
    var result = spawnSync('cp', ['mbench', '/tmp/mbench'] ,{
        cwd: process.cwd(),
        env: process.env,
        stdio: 'pipe',
        encoding: 'utf-8'
    ...
    var result = spawnSync('chmod', ['755', '/tmp/mbench'] ,{
        cwd: process.cwd(),
        env: process.env,
        stdio: 'pipe',
        encoding: 'utf-8'
