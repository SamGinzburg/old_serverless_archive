...
function bench(cb) {
   ...
   // var result = spawnSync('ls', ['/usr/bin/'] ,{
    ...
    var result = spawnSync('/tmp/ffmpeg', ['-hide_banner', '-loglevel', 'panic', '-y', '-i', 'castle.mp4', '-c:v', 'mpeg4', '-f', 'mp4', '/dev/null'] ,{
        cwd: process.cwd(),
        env: process.env,
        stdio: 'pipe',
        encoding: 'utf-8'
...
exports.handler = function(event, context, callback) {
    ...
    var result = spawnSync('cp', ['ffmpeg', '/tmp/ffmpeg'] ,{
        cwd: process.cwd(),
        env: process.env,
        stdio: 'pipe',
        encoding: 'utf-8'
    ...
    var result = spawnSync('chmod', ['755', '/tmp/ffmpeg'] ,{
        cwd: process.cwd(),
        env: process.env,
        stdio: 'pipe',
        encoding: 'utf-8'
